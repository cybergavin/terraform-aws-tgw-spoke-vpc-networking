variable "org" {
  ...
  validation {
    ...
    error_message = "var.org accepts only alphanumeric characters and hyphens, with a string length from 3 to 8 characters. The value of var.org must not begin with a hypen and must not contain consecutive hyphens."
  }
}
...
variable "app_id" {
  ...
  validation {
    ...
    error_message = "var.app_id accepts only alphanumeric characters, with a string length from 3 to 8 characters."
  }
}
...
variable "environment" {
  ...
  validation {
    condition     = contains(["poc", "dev", "tst", "stg", "prod"], var.environment)
    error_message = "var.environment must be one of: poc, dev, tst, stg, prod."
  }
}
...
variable "vpc_cidr" {
  ...
  validation {
    ...
    error_message = "var.vpc_cidr must be a valid CIDR block in the form of X.X.X.X/Y where X is between 0 and 255 and Y is between 0 and 32."
  }
}
